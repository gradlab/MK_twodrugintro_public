...
* `sens_analysis_figures`: Contains the figures that go on to form Figure 3 of the main text. Subfolder `prop_plots` contains the figures that form Supplementary Figure 6 and Supplementary Figure 7. <br> <br>

__Output__: \
Outputs are as described in the code descriptions. Briefly:
* `2025-06-29ea_unsens`: Contains the dataframe output from `code/unsensitivity_analysis_ea.R` which are then used to generate the plots in Supplementary Figure 5.
* `2025-06-29seq_unsens`: Contains the dataframe output from `code/unsensitivity_analysis_seq.R` which are then used to generate the plots in Supplementary Figure 5.
* `baseline_parms_analysis`: contains the .csv output files from running the stochastic models using baseline parameters and varying the resistance prevalence threshold. These outputs are used to generate the plots which become Figures 1 and 2, and Supplementary Figures 3 & 4.
* `calibration`: the calibration output files as described in code above
* `deterministic_output`: the output files from the deterministic models as described in code above.
* `sens_anal_outputs`: The proportion dataframes generated from `sens_analysis_propplots.R` that track the resistance prevalence trajectories for the 22 edge cases.
* `unsensitivity_analysis_outputs`: Outputs generated by `unsensitivity_analysis_output_plots.R` that summarize the sensitivity analysis of Group 1 parameters.
