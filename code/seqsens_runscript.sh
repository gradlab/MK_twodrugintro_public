...
#SBATCH -o logs/seqsens_%j.out  # File to which STDOUT will be written, %j inserts jobid
#SBATCH -e logs/seqsens_%j.err  # File to which STDERR will be written, %j inserts jobid
...
# Run the R script
singularity exec --cleanenv --env R_LIBS_USER=${my_packages} ${rstudio_singularity_image} Rscript seqsens_runscript.R
